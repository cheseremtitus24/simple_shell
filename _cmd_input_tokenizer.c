...
bool
ReportError(char *filepath, char **argv)
{
	struct stat sb;
...
/*        perror(filepath);*/
	...
	} else
	{
		...
		{
			case S_IFBLK:
				printf("%s: %s: Is a block device\n", argv[0], filepath);
				break;
			case S_IFCHR:
				printf("%s: %s: Is a character device\n", argv[0], filepath);
				break;
			case S_IFDIR:
				printf("%s: %s: Is a directory\n", argv[0], filepath);
				break;
			case S_IFIFO:
				printf("%s: %s: Is a FIFO/pipe\n", argv[0], filepath);
				break;
			case S_IFLNK:
				printf("%s: %s: Is a symlink\n", argv[0], filepath);
				break;
			case S_IFREG:
				printf("%s: %s: Is a regular file\n", argv[0], filepath);
				break;
			case S_IFSOCK:
				printf("%s: %s: Is a socket\n", argv[0], filepath);
				break;
			default:
				printf("%s: %s: Is unknown?\n", argv[0], filepath);
				break;
		}
